...
export const Default: Story = {
  render: () => (
    <Popover>
      <PopoverTrigger className="text-sm font-medium text-foreground hover:text-primary">
        Open Popover
      </PopoverTrigger>
      ...
      <PopoverContent placement="bottom" className="p-2">
        {({ close }) => (
          <div className="flex flex-col gap-1">
            <button
              type="button"
              className="px-3 py-1.5 text-sm rounded hover:bg-muted text-left"
              onClick={close}
            >
              Option A
            </button>
    ...
    return (
      <div className="flex items-center gap-3">
        <Popover>
          <PopoverTrigger>
            <span
              className="inline-block w-6 h-6 rounded-full border-2 border-border"
