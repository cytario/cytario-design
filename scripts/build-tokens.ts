...
// --- Generate Tailwind v4 @theme inline utilities for semantic tokens ----
// Each flat semantic token gets a `--color-<name>: var(--color-<name>)` entry.
// `@theme inline` makes utilities emit `var(--color-<name>)` (not a frozen
// hex), so `bg-background`, `text-destructive`, … stay theme-reactive against
// the values in variables.css / variables-dark.css.
// Nested groups (badge/delta/progress) and the primitive scales are excluded
// on purpose — they keep their arbitrary-syntax (`bg-(--color-badge-purple-bg)`)
// and primitive utilities respectively.
const flatTokens = Object.entries(lightJson.color)
  .filter(([k, v]) => !k.startsWith("$") && v && typeof v === "object" && "$value" in v)
  .map(([k]) => k);

const themeBlock = flatTokens.map((n) => `  --color-${n}: var(--color-${n});`).join("\n");
const utilities =
  `/**\n * AUTO-GENERATED by scripts/build-tokens.ts — do not edit.\n` +
  ` * Idiomatic Tailwind v4 utilities for the semantic color tokens.\n */\n\n` +
  `@theme inline {\n${themeBlock}\n}\n`;
await writeFile(join(CSS_BUILD_PATH, "utilities.css"), utilities);
console.log(`Design tokens built: utilities.css (${flatTokens.length} semantic @theme inline utilities).`);
